...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-4, 3, 0));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 0, 0));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(5, 3, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_K)) {
        if (isPerspective) {
            ...
            ortho_height -= 10 * deltaTime;

			projectionMatrix = glm::ortho(-ortho_height * window->props.aspectRatio, ortho_height * window->props.aspectRatio, -ortho_height, ortho_height, 0.01f, 200.0f);
        }
    }
